...
test('all', (t) => {
    t.test('can find highest digit [1,2,3]', () => {
        let [startDigit, index] = findHighestDigitForWidth([1,2,3], 1)
        assert.strictEqual(startDigit, 3)
        assert.strictEqual(index, 2)
    });

    t.test('can find highest digit [3,2,1]', () => {
        let [startDigit, index] = findHighestDigitForWidth([3,2,1], 1)
        assert.strictEqual(startDigit, 3)
    ...
    t.test('can find max big num', () => {
        const input = toBattery(2231336232333236334112243332332341353232312512325222263432225332633322235632375232223315283333433363n)
        const joltage = maxLargeJoltage(input)
        assert.strictEqual(joltage, 888911112111)
    })
})
